...
## Комагдная работа в Git

### Алгоритм для создания PR
1. Склонировать репозиторий.
	1. Если вы не участник проекта, предварительно сделать «форк» исходного репозитория.
	1. На странице репозитория или «форка» нажать кнопки: Code → SSH → скопировать ссылку.
	1. Выполнить команду git clone <ссылка на репозиторий>.
1. Создать ветку для вашей задачи: git checkout -b my-task-branch-name.
1. Добавить и «закоммитить» изменения, которые вы хотите внести в проект.
1. «Запушить» ветку:
```bash
...
```
	...
	1. PR можно также создать через интерфейс GitHub.
...
1. Ещё раз обновить main, чтобы «подтянуть» ваши изменения в основную ветку локального репозитория: git checkout main && git pull.

### Алгоритм для разрешения конфликтов слияния
1. Открыть проект в IDE (VS Code, IDEA или другие).
1. Открыть файл, в котором есть конфликт.
1. Выбрать, какие части файла нужно взять из одной ветки, а какие — из другой.
1. Когда конфликты разрешены, сделать коммит:
```bash
	git commit --no-edit
```
или
